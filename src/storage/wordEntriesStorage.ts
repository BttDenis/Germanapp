...
const normalizeImportedEntry = (entry: Partial<WordEntry>): WordEntry | null => {
  ...
  return {
    ...
    notes: entry.notes ?? "",
    nounPlural: entry.nounPlural ?? null,
    nounGenitive: entry.nounGenitive ?? null,
    verbThirdPerson: entry.verbThirdPerson ?? null,
    verbPast: entry.verbPast ?? null,
    verbParticipleIi: entry.verbParticipleIi ?? null,
    verbAuxiliary: entry.verbAuxiliary ?? null,
    adjectiveComparative: entry.adjectiveComparative ?? null,
    adjectiveSuperlative: entry.adjectiveSuperlative ?? null,
...
export const mergeWordEntries = (incoming: Partial<WordEntry>[]) => {
  ...
  normalizedIncoming.forEach((entry) => {
    ...
    const nextEntry: WordEntry = {
      ...
      nounPlural: existingEntry.nounPlural ?? entry.nounPlural ?? null,
      nounGenitive: existingEntry.nounGenitive ?? entry.nounGenitive ?? null,
      verbThirdPerson: existingEntry.verbThirdPerson ?? entry.verbThirdPerson ?? null,
      verbPast: existingEntry.verbPast ?? entry.verbPast ?? null,
      verbParticipleIi: existingEntry.verbParticipleIi ?? entry.verbParticipleIi ?? null,
      verbAuxiliary: existingEntry.verbAuxiliary ?? entry.verbAuxiliary ?? null,
      adjectiveComparative: existingEntry.adjectiveComparative ?? entry.adjectiveComparative ?? null,
      adjectiveSuperlative: existingEntry.adjectiveSuperlative ?? entry.adjectiveSuperlative ?? null,
