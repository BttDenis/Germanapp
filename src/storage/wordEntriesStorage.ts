...
export const exportWordEntries = () => {
  return JSON.stringify(getWordEntries(), null, 2);
};

export const importWordEntries = (payload: string) => {
  let parsed: WordEntry[] = [];
  try {
    const raw = JSON.parse(payload) as WordEntry[];
    if (!Array.isArray(raw)) {
      throw new Error("Sync code must be a JSON array.");
    }
    parsed = raw;
  } catch (error) {
    ...
    throw new Error("Sync code is invalid JSON.");
  }
  return mergeWordEntries(parsed);
};
