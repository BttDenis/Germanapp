...
> ⚠️ **Important:** Any `VITE_*` values are embedded in the client build. Do **not** set
> `VITE_LLM_API_KEY` (or other secrets) when deploying to GitHub Pages, or the key will
> be published in the `gh-pages` branch and blocked by GitHub's secret scanning. For
> production, use a server-side proxy and keep secrets on the server only.

### Production sync + MongoDB-backed image storage

If you want word sync between devices and MongoDB-backed image storage in a production
deployment, both are already supported by the client. You just need to provide the
appropriate URLs and tokens at build time:

1. **Word sync server**
   - Deploy the included sync server (`scripts/word-sync-server.mjs`) to a public host
     and set the environment variables listed below.
   - Add these to your Vite `.env` (they become `import.meta.env` at build time):

     ```bash
     VITE_WORD_SYNC_URL=https://your-sync.example.com/words
     VITE_WORD_SYNC_TOKEN=your-shared-token
     ```
   - For MongoDB persistence (stores the current synced list and keeps a history of
     generations/regenerations), run the sync server with:

     ```bash
     WORD_SYNC_MONGODB_URI=mongodb+srv://<user>:<pass>@cluster/db
     WORD_SYNC_DB_NAME=germanapp
     WORD_SYNC_COLLECTION=wordEntries
     WORD_SYNC_HISTORY_COLLECTION=wordEntryHistory
     ```

2. **MongoDB-backed image storage**
   - Deploy an image upload API that stores metadata in MongoDB and returns a URL.
...
> ✅ These settings are already wired into the app. No extra code changes are required
> for the client; just deploy the services and set the environment values before you build.
